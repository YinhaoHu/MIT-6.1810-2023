...
e1000_transmit(struct mbuf* m) {
  ...
  if (index >= TX_RING_SIZE) {
    printf("tx ring overflowing\n");
    release(&e1000_lock);
    return -1;
  }

  // previous transmission request has not been done yet.
  if ((tx_ring[index].status & E1000_TXD_STAT_DD) == 0) {
    printf("unfinished request remains\n");
    release(&e1000_lock);
    return -1;
  }

  // free the last mbuf (if exists one).
  struct mbuf* last_mbuf = tx_mbufs[index];
  if (last_mbuf != 0) {
    mbuffree(last_mbuf);
  }
