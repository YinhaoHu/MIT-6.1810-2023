...
barrier() {
  ...
  // In: last in, broadcast
  // Out: first out, incr round.
  int init_round = bstate.round;

  assert(pthread_mutex_lock(&bstate.barrier_mutex) == 0);
  bstate.nthread += 1;
  if (bstate.nthread == nthread * (init_round + 1)) {
    assert(pthread_cond_broadcast(&bstate.barrier_cond) == 0);
  } else {
    while (bstate.nthread < nthread * (init_round + 1)) {
